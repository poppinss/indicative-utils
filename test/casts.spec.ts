...
test.group('toFloat', () => {
  test('return number as it is', (assert) => {
    assert.equal(cast(10, 'float'), 10)
  })

  test('keep decimal on numbers', (assert) => {
    assert.equal(cast(10.12, 'float'), 10.12)
  })
  ...
  test('keep decimal on string counterparts', (assert) => {
    assert.equal(cast('10.60', 'float'), 10.60)
  })

  test('return null for non-castable types', (assert) => {
    assert.isNull(cast('hello', 'float'))
    assert.isNull(cast({}, 'float'))
    assert.isNull(cast([], 'float'))
    assert.isNull(cast(null, 'float'))
    assert.isNull(cast(undefined, 'float'))
    assert.isNull(cast(new Date(), 'float'))
    assert.isNull(cast(true, 'float'))
  })
})
